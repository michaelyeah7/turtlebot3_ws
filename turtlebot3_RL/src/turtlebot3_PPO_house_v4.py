...
from environment_PPO_v4 import Env
...
class PPO(object):
    ...
    def __init__(self,a_lr,c_lr,s_dim,a_dim,aSteps,cSteps,method,gpu_options,complexNN):
        ...
        self.tfs = tf.placeholder(tf.float32, [None, self.S_DIM,3], 'state')
        self.tfs_others = tf.placeholder(tf.float32,[None,4])
        ...
        with tf.variable_scope('critic'):
            ...
            l2 = None
            if(self.complexNN):
                hid1_size = 50  # first layer
                hid2_size = 100#int(np.sqrt(hid1_size * hid3_size)) # second layer
                # l1 = tf.layers.dense(self.tfs, hid1_size, tf.nn.relu,trainable=True) 
                # l2 = tf.layers.dense(l1, hid2_size, tf.nn.relu,trainable=True)

                l1 = tf.layers.conv1d(self.tfs,filters=4,kernel_size=5,strides=2, padding="valid", activation=tf.nn.relu)
                l2 = tf.layers.conv1d(l1,filters=4,kernel_size=3,strides=2, padding="valid", activation=tf.nn.relu)
                l3 = tf.layers.dense(l2,32,tf.nn.relu,trainable=True)
                l4 = tf.concat(l3,self.tfs_others)
                l5 = tf.layers.dense(l4,16)
            else:
                l5 = tf.layers.dense(self.tfs, 100, tf.nn.relu)
    ...
    def update(self, s, a, r):
        s_scan = s[:len(s)-4].copy()
        s_other = s[-4:].copy()
        s_scan = s_scan[np.newaxis, :]
        s_other = s_other[np.newaxis, :] 

        self.sess.run(self.update_oldpi_op)
        adv = self.sess.run(self.advantage, {self.tfs: s_scan, self.tfs_others: s_other, self.tfdc_r: r})
        ...
        if self.METHOD['name'] == 'kl_pen':
            for _ in range(self.A_UPDATE_STEPS):
                _, kl = self.sess.run(
                    ...
                    {self.tfs: s_scan, self.tfs_others: s_other, self.tfa: a, self.tfadv: adv, self.tflam: self.METHOD['lam']})
            ...
            [self.sess.run(self.atrain_op, {self.tfs: s_scan, self.tfs_others: s_other, self.tfa: a, self.tfadv: adv}) for _ in range(self.A_UPDATE_STEPS)]

        # update critic
        [self.sess.run(self.ctrain_op, {self.tfs: s_scan, self.tfs_others: s_other, self.tfdc_r: r}) for _ in range(self.C_UPDATE_STEPS)]

    def _build_anet(self, name, trainable):
        #################################three layer added by qinjielin##################################
        hid1_size = 50#self.S_DIM * 6  # first layer
        hid2_size = 20#int(np.sqrt(hid1_size * hid3_size)) # second layer
        hid3_size = 100  # third layer 
        with tf.variable_scope(name):
            ...
            if(self.complexNN):
                ...
                l5 = tf.layers.dense(self.tfs, 100, tf.nn.relu, trainable=trainable)
            mu = 2 * tf.layers.dense(l5, self.A_DIM, tf.nn.tanh, trainable=trainable)
    ...
    def choose_action(self, s):
        ...
        a = self.sess.run(self.sample_op, {self.tfs: s_scan, self.tfs_others: s_other})[0]
        if(itimateFlag):
            tempa = getTeleopCMd()
            a[0] = tempa[0]
            a[1] = tempa[1]
    ...
    def get_v(self, s):
        ...
        s_scan = s_scan[np.newaxis, :]
        s_other = s_other[np.newaxis, :]
        return self.sess.run(self.v, {self.tfs: s_scan, self.tfs_others: s_other})[0, 0]
...
if __name__ == "__main__":
    ...
    fileLoc = os.path.dirname(os.path.abspath(__file__))+"/../../../../../../clever/saved_model_guo/"
    ...
    for ep in range(EP_MAX):
        ...
        #save ep reward data
        if(trainFlag):
            writeData(fileLoc,filename,ep,all_ep_r[-1],env.numSuccess)
        print(
            'Ep: %i' % ep,
            "|Ep_r: %i" % ep_r,
