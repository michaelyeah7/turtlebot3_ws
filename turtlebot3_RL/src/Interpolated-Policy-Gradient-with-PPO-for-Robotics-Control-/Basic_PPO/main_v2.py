...
sys.path.append('/root/control_ws/src/turtlebot3_RL/src/Interpolated-Policy-Gradient-with-PPO-for-Robotics-Control-')
sys.path.append('/root/control_ws/src/turtlebot3_RL/src')
...
def run_episode(env, policy, scaler, animate=False):
    ...
    #guo changed here, do not scale or offset the state done
    #scale[-1] = 1.0  # don't scale state done feature
    #offset[-1] = 0.0  # don't offset state done feature    

    # scale[-1] = 1.0  # don't scale time step feature
    # offset[-1] = 0.0  # don't offset time step feature

    while not done and step<100:
        #guo changed here, we do not need to use gym to render
        # if animate:
        #     env.render()
        ...
        #guo changed here, if collision or reach goal, reset env
        if done:
            env.reset()
        #print("reward",reward)
...
def main(num_episodes, gamma, lam, kl_targ, batch_size, env_name, monitor=False):
    ...
    #guoye, we do not add one more dimension
