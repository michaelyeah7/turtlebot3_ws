...
void ActorPlugin::ChooseNewTarget()
{
  ...
  while ((newTarget - this->target).Length() < 3.0)
  {
    gzdbg << "creating new target" << std::endl;
    gzwarn << "creating new target" << std::endl;
    ROS_INFO("creating new target");
    newTarget.X(ignition::math::Rand::DblUniform(-10.0, 10.0));
    newTarget.Y(ignition::math::Rand::DblUniform(-10.0, 10.0));
...
{
  gzdbg << "handling obtacles" << std::endl;
  gzwarn << "handling obtacles" << std::endl;
  for (unsigned int i = 0; i < this->world->ModelCount(); ++i)
  {
